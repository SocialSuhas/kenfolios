...
<body style="margin:0;padding:0;background:#f6f6f6;font-family:Arial,sans-serif;color:#333;">
  <div style="max-width:600px;margin:0 auto;background:#ffffff;padding:30px;border-radius:6px;">
    ...
    <div style="font-size:12px;text-align:center;color:#777;margin-top:35px;">
      © ${new Date().getFullYear()} KenFolios Media Private Limited. All rights reserved.
    </div>
  </div>
</body>
</html>`
    ...
    try {
      ...
      if (
        ...
        if (update.chat_join_request) {
          ...
          if (!isPaid) {
            ...
            try {
              await sendDM(
                user.id,
                "We could not approve your join request.\n\nPlease ensure your membership is activated using the link shared after payment, or contact support if you’ve already paid."
              );
            } catch (e) {
          ...
          try {
            await sendDM(
              user.id,
              "🎉 <b>Your membership is verified and your join request has been approved.</b>\n\nWelcome to the KenFolios Business Community!"
            );
          } catch (e) {
        ...
        if (!msg || !msg.from || !msg.chat || msg.chat.type !== "private") {
          // Not a private chat message — just ack
          return new Response(JSON.stringify({ ok: true }), {
            status: 200,
            headers: { "Content-Type": "application/json" }
          });
